...
void schedule()
{
    ...
    int compTime = 0, count = 0, wait = 0, turn = 0, res = 0, currPriority = 0;

    while (otherHead != NULL)
    {
        sort(&otherHead, 1);
        temp = otherHead;
        currPriority = temp->task->priority;

        while (temp != NULL)
        {
            if (currPriority == temp->task->priority) {
                ...
                // if (prevHead != NULL && prevHead->next != NULL && temp->next != NULL && temp->task->tid == prevHead->task->tid ) {
                //     printf("yolo\n");
                // }
